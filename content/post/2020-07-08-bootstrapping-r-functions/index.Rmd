...
1. Capture my setup code, and evaluate it at the top of my new bootstrapping function.
...
bootstrapping_function <- function(fn, setup) {
  ...
  intended_function_body <- body(fn)
  bootstrapping_function <- fn
  body(bootstrapping_function) <- substitute({
    ...
    get_args <- function() { # https://stackoverflow.com/a/47955845/8456369
    ...
    assign(this_function_name, fn, this_function_env)
    do.call(
      this_function_name,
      args = get_args(),
...
add_1_to_all_numeric_columns <- bootstrapping_function(
  function(df) mutate_if(df, is.numeric, function(x) x + 1),
  setup = {
...
```{r eval-first-time}
add_1_to_all_numeric_columns(mtcars)
```
