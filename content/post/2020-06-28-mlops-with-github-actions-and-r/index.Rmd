...
For the most part, this action runs silently. If there's an error in the check, such as a failing unit test or the package can't be installed, then I'll get an email about it. I could even set up a requirement that this check must pass before I merge into the _production_ branch.
...
1. Run `drake::make` on the training plan
1. Maintain a text file `bot-comment.txt` which contains the markdown for the pull request comment
1. Upload the ggplot to S3
1. Post the comment
...
I do like the approach that mlflow takes to serving R models through the `carrier` package: the user declares a predict function, along with a list of required artefacts. It's a little more fiddly than just uploading a scikit-learn object, and I haven't properly tested its limitations, but I _much prefer_ the idea behind this approach. There's plenty of scope here for automated dependency detection, just like how `drake` does it, and it's as flexible as it gets.
