...
```{r log-metrics}
log_metrics <- function(metrics, estimate = "standard") {
  metrics %>% filter(.estimator == estimate) %>% pmap(
...
There's one last component I need to make this work. Apart from parameters and metrics, I can also store _artifacts_ with each run. These are usually models, but could be anything. MLflow supports exporting models with the `carrier::crate`. [This is a tricky function to use](/post/deploying-r-models-with-mlflow-and-docker/), since the user must comprehensively list their dependencies. For a `workflow` with a `recipe`, it's a lot easier. All of the preprocessing is contained within the recipe, and the fitted workflow object contains this.
...
I've written before about data leakage, and how easy it is to do. I can't use the same test data to select my hyperparameters _and_ evaluate my model, because this leaks information from the test set to the model. But the UI places the hyperparameters next to the metrics, making it look as though I should be selecting the hyperparameters with the best metrics. It's not a big deal, and it's not a flaw of MLflow in particular, but it is something to not fall for.
